    ...
    <header className="sticky top-0 z-50 bg-white/95 backdrop-blur border-b border-slate-200">
      ...
      {mobileOpen && (
        <nav className="lg:hidden border-t border-slate-100 bg-white px-3 py-2">
          ...
          <Link href="/offers" onClick={() => setMobileOpen(false)} className="flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-700"><Gift size={18} /> Offers</Link>
          <Link href="/help" onClick={() => setMobileOpen(false)} className="flex items-center gap-2.5 rounded-lg px-3 py-2.5 text-sm font-medium text-slate-700"><HelpCircle size={18} /> Help</Link>
        </nav>
      )}
    </header>
  );
