...
const MONTHS = ["Flexible", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];

export function EnquiryModal({ pkg, open, onClose }: { pkg: Package; open: boolean; onClose: () => void }) {
  const { user } = useAuth();
  const [form, setForm] = useState({ name: "", email: "", phone: "", travelMonth: "Flexible", adults: 2, children: 0, message: "" });
        ...
        <form onSubmit={submit}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="Travel month">
              <select value={form.travelMonth} onChange={(e) => set("travelMonth", e.target.value)} className="einp">{MONTHS.map((m) => <option key={m}>{m}</option>)}</select>
            </Field>
