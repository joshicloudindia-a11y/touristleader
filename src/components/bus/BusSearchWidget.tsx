...
export function BusSearchWidget() {
  ...
  const search = () => router.push(`/bus/search?from=${encodeURIComponent(from)}&to=${encodeURIComponent(to)}&date=${date}`);
      ...
      <div className="px-4 pt-4 sm:px-6">
        ...
        <div className="grid grid-cols-1 divide-y divide-slate-200 rounded-xl border border-slate-200 sm:grid-cols-2 sm:divide-y-0 md:grid-cols-12 md:divide-x">
          <div className="relative grid grid-cols-2 border-b border-slate-200 sm:border-b-0 md:col-span-7">
            <div className="min-w-0 border-r border-slate-200 pr-5"><BusCitySelect label="From" value={from} onChange={setFrom} /></div>
            <div className="min-w-0 pl-5"><BusCitySelect label="To" value={to} onChange={setTo} /></div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          {BUS_TRENDING.map((t) => (
            <button key={`${t.from}-${t.to}`} onClick={() => { setFrom(t.from); setTo(t.to); }} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600 hover:bg-brand/10 hover:text-brand">{t.from} → {t.to}</button>
          ))}
        </div>
      </div>
