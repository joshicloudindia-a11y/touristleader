...
interface Booking { id: string; bookingRef: string; pnr: string | null; bookingType: string; status: string; paymentStatus: string; origin: string; destination: string; departDate: string; totalAmount: number; contactEmail: string; contactPhone: string; createdAt: string; cabinClass: string }
        ...
        <div className="overflow-x-auto rounded-2xl bg-white shadow-sm ring-1 ring-slate-100">
          <table className="w-full min-w-[760px] text-sm">
            <thead><tr className="border-b border-slate-100 text-left text-[11px] font-bold uppercase tracking-wide text-slate-400">
              <th className="px-4 py-3">Booking</th><th className="px-4 py-3">Type</th><th className="px-4 py-3">Route</th><th className="px-4 py-3">Customer</th><th className="px-4 py-3">Date</th><th className="px-4 py-3 text-right">Amount</th><th className="px-4 py-3">Status</th>
            </tr></thead>
            <tbody>
              ...
              {filtered.length === 0 && <tr><td colSpan={7} className="px-4 py-10 text-center text-slate-400">No bookings found.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
