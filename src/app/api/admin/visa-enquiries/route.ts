...
export async function GET() {
  ...
  const enquiries = await prisma.visaEnquiry.findMany({ orderBy: { createdAt: "desc" }, take: 300 });
...
export async function PATCH(req: NextRequest) {
  ...
  if (status !== undefined) { if (!VISA_STATUSES.includes(status)) return NextResponse.json({ error: "Invalid status" }, { status: 400 }); data.status = status; }
  if (note !== undefined) data.note = (note || "").trim() || null;
  if (!Object.keys(data).length) return NextResponse.json({ error: "Nothing to update" }, { status: 400 });
  await prisma.visaEnquiry.update({ where: { id }, data });
