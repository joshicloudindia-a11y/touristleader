...
export async function GET() {
  ...
  const list = await Promise.all(agents.map(async (a) => {
    ...
    return { id: a.id, name: a.name || a.email.split("@")[0], email: a.email, state: a.state, balance: a.wallet?.balance || 0, totalEarned: earned._sum.amount || 0, bookings };
  }));
...
export async function POST(req: NextRequest) {
  ...
  const wallet = await prisma.wallet.findUnique({ where: { userId: agentId } });
  const amount = Math.round(wallet?.balance || 0);
  ...
  // mark pending commission settled + debit wallet to zero
  await prisma.walletTransaction.updateMany({ where: { userId: agentId, reason: "COMMISSION", status: "PENDING" }, data: { status: "SETTLED" } });
  await walletTxn({ userId: agentId, type: "DEBIT", amount, reason: "SETTLEMENT", refType: "SETTLEMENT", note: `Settled by admin${note ? ` · ${note}` : ""}` });
